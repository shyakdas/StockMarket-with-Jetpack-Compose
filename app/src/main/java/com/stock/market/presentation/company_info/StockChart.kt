...
fun StockCharts(
    ...
    Canvas(modifier = modifier) {
        val spacePerHours = (size.width - spacing) / infos.size
        // X Axis
        (0 until infos.size - 1 step 2).forEach { i ->
            val info = infos[i]
            val hour = info.date.hour
            drawContext.canvas.nativeCanvas.apply {
                drawText(hour.toString(), spacing + i * spacePerHours, size.height - 5, textPaint)
            }
        }
        // Y Axis
        ...
        val strokePath = Path().apply {
            ...
            for (i in infos.indices) {
                ...
                val x1 = spacing + i * spacePerHours
                val y1 = height - spacing - (leftRatio * height).toFloat()
                val x2 = spacing + (i + 1) * spacePerHours
